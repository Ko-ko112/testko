...
export default class CartScreen extends Component {
  ...
  getShopFromApi = filter => {
    ...
    api
      .getOrderByShopId(bodyParameters)
      .then(Response => {
        this.setState({
          orderData: [],
          isLoading: false,
  ...
  renderItem = ({item, index}) => {
    return (
      <View key={index} style={styles.itemContainer}>
        ...
        <View style={styles.itemBody}>
          <Text style={styles.itemName}>20 แต้ม</Text>
          <Text style={styles.itemName}>ราคา 100 ฿</Text>
          <Text style={styles.itemsubName}>สถานะ {item.status}</Text>
        </View>
    ...
    return (
      <SafeAreaView style={styles.container}>
        ...
        <View
          ...
          <Text style={{fontSize: 18, fontWeight: 'bold'}}>100 บาท</Text>
        </View>
        ...
        <View style={{marginHorizontal: 50, marginBottom: 20}}>
          <Button
            title="สั่งซื้อ"
            color="#FC934F"
            onPress={() => this.props.navigation.navigate('QReBeSim')}
          />
        </View>
      </SafeAreaView>
